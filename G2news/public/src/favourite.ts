...
function toggleFavourite(event: any) {
    if (event.target.matches('.fav-icon')) {
        ...
        const existingIndex = favouritesData.findIndex((data: CardData) => data.title === cardTitle);
...
export function updateFavoriteIcons() {
    const favButtons = document.querySelectorAll<HTMLButtonElement>('.fav-icon');
    const favouritesData = JSON.parse(localStorage.getItem('favourites') || '{}');

    favButtons.forEach((button: HTMLButtonElement)=> {
        const cardTitle  = button.closest('.news-card')?.querySelector('.card-title')?.textContent;
        const isFavourite = favouritesData.some((data : CardData) => data.title === cardTitle);
        if (isFavourite) {
...
function displayFavourites() {
    ...
    if (favouritesContainer) {
        ...
        favouritesData.forEach((cardData : CardData ) => {
            const cardHtml = `
